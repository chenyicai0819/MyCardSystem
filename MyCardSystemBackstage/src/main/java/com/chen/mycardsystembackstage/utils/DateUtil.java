...
public class DateUtil {
    ...
    public static String getAgeByBirthday(String date) {
        Date date1 = DateUtil.toDateYMD(date);
        String birthdayStr = DateUtil.toStringYMD(date1);
    ...
    public static String getAgeByCreateTime(Date date, Date createTime) {
        String birthdayStr = DateUtil.toStringYMD(date);
        if (StringUtils.isNotBlank(birthdayStr)) {
            String[] data = birthdayStr.split("-");
     ...
     * @return
     */
    public static String getHoursByDay(Date startTime , Date endTime){

        String birthdayStr = DateUtil.toStringYMDHMS(endTime);
        if (StringUtils.isNotBlank(birthdayStr)) {
            ...
            int day = end.get(Calendar.DAY_OF_MONTH) - now.get(Calendar.DAY_OF_MONTH);
            int month = end.get(Calendar.MONTH) - now.get(Calendar.MONTH);
            int year = end.get(Calendar.YEAR) - now.get(Calendar.YEAR);
            int hour = end.get(Calendar.HOUR_OF_DAY) - now.get(Calendar.HOUR_OF_DAY);
            int minute = end.get(Calendar.MINUTE) - now.get(Calendar.MINUTE);
            int second = end.get(Calendar.SECOND) - now.get(Calendar.SECOND);

            StringBuilder tag = new StringBuilder();
            int hourInt = 0;
            if (year > 0) {
                hourInt += year*12*30*24;
            }
            if (month > 0) {
                hourInt += month*30*24;
            }
            ...
            if (hour >= 0){
                hourInt+= hour;
                // tag.append(hourInt).append("小时");
            }
            if (minute >= 0){
                minute = minute;
                // tag.append(minute).append("分");
            }else {
                hourInt -= 1;
                minute = 60 + minute;
            }
            if (second >= 0){
                second = second;
                // tag.append(second).append("秒");
            }else {
                minute -= 1;
                second = 60 + second;
            }
            tag.append(hourInt).append("小时").append(minute).append("分").append(second).append("秒");
            return String.valueOf(tag);
        } else {
            return "00小时00分00秒";
        }
